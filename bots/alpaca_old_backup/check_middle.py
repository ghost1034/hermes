...
# Sort ascending by PNL
sorted_pnl = sorted(pnl_by_symbol.items(), key=lambda x: x[1])

# Find the middle 5
total = len(sorted_pnl)
mid_start = (total // 2) - 2
mid_end = mid_start + 5
middle_5 = sorted_pnl[mid_start:mid_end]

print(f"Total symbols traded: {total}")
print("Middle 5 Symbols:")
for symbol, pnl in middle_5:
